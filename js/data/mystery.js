...
const MYSTERIES = [
  ...
  },
  {
    ...
    choices: [
      { label: 'Hand it straight back', apply: (s, log) => { s.karma += 3; s.money += 22; s.buffs.mult = (s.buffs.mult || 0) + 1; log('He presses $22 on you and will not take no. (+1 Mult)'); giveContact('vendor', log); } },
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Buy the whole tray ($9)', req: s => s.money >= 9,
        apply: (s, log) => { s.money -= 9; s.members.forEach(m => m.stamina = 100); s.buffs.crowd = (s.buffs.crowd || 1) * 1.3; log('He wraps the lot. The band eats like kings and plays like it. (Full stamina, bigger crowd next set)'); giveContact('moth', log); } },
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Play together instead', apply: (s, log) => { s.karma += 2; s.money += 7; const t = 7; s.buffs.crowd = (s.buffs.crowd || 1) * 1.25; log('Two acts, one tune. The tunnel fills. You split ' + fmtMoney(t * 2) + '.'); giveContact('rival', log); } },
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Play for him instead', game: 'copy', gameLabel: 'MATCH THE BELL',
        good: (s, log) => { s.karma += 3; s.members.forEach(m => m.stamina = Math.min(100, m.stamina + 20)); log('He rings, you answer, for a long time. You leave lighter. (+20 stamina each)'); giveContact('monk', log); },
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Tear it up in front of her', apply: (s, log) => { s.karma += 2; s.buffs.mult = (s.buffs.mult || 0) + 1; log('She watches the pieces fall. "Good." She means it. (+1 Mult)'); giveContact('scout', log); } },
    ],
  },
  {
    id: 'rain',
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'Ask him what he plays', game: 'copy', gameLabel: 'HE HUMS IT - PLAY IT BACK',
        good: (s, log) => { s.karma += 2; s.money += 12; s.buffs.crowd = (s.buffs.crowd || 1) * 1.2; log('Trumpet, police band, 1988. He requests one and tips $12. Nobody moves you on again tonight.'); giveContact('officer', log); },
