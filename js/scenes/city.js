...
class CityScene {
  ...
  draw(ctx) {
    ...
    // the rest of the band trail behind in a neat little queue
    const dirX = this.travel ? (this.travel.to.x >= this.travel.from.x ? 1 : -1) : -1;
    r.members.slice(1, 4).forEach((m, i) => {
      const fx2 = ps.x - dirX * (26 + i * 16), fy2 = ps.y + 9 + Math.sin(this.t * 4.5 + i * 1.3) * 1.5;
      drawShadow(ctx, fx2, fy2 + 1, 15, 0.2);
      drawBugAt(ctx, m.spec, fx2, fy2, { pose: Math.floor(this.t * 5 + i) % 2 ? 'walk1' : 'walk2', flip: dirX > 0, scale: 0.58, rate: 3.4, phase: i * 1.7 });
    });
