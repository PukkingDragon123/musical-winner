...
function buildPads(instr, area, qte) {
  ...
  switch (instr.game) {
    // A kit has no pads at all: you tap the drum itself.
    case 'lanes': { if (instr.noPads || instr.view === 'kit') return pads;
      const L = instr.lanes;
      for (let i = 0; i < L; i++) add(i / L, 1 / L, instr.keys[i], instr.keyNames[i], LANE_COLORS[i % LANE_COLORS.length]); break; }
